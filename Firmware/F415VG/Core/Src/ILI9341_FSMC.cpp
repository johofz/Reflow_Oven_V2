...
uint16_t ILI9341::width = 240;
uint16_t ILI9341::height = 320;
uint32_t ILI9341::pixelCount = width * height;

void ILI9341::SendCommand(uint16_t com)
{
    *(__IO uint16_t*) (0x60000000) = com;
}

void ILI9341::SendData(uint16_t data)
{
...
void ILI9341::Reset()
{
    TFT_RST_GPIO_Port->BSRR = (uint32_t) TFT_RST_Pin << 16U;    // RST Low
    HAL_Delay(100);
    TFT_RST_GPIO_Port->BSRR = TFT_RST_Pin;    // RST High
}
...
void ILI9341::Image(uint16_t x, uint16_t y, uint16_t w, uint16_t h, const uint16_t *data,
        uint32_t size)
{
    uint32_t n = size;
